    ...
    func updateGameState() {
        ...
        if state == .Winner{
            ...
            if let winningLabel = winningLabel as? SKLabelNode,
                let player1_score = self.scene?.childNode(withName: "//playerOneScore") as? SKLabelNode,
                let player2_score = self.scene?.childNode(withName: "//playerTwoScore") as? SKLabelNode{
